...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while True:
            if s == beehive:
                return None
            if s.bees and self.min_range <= distance_away <= self.max_range:
                return rANTdom_else_none(s.bees)
            s = s.entrance
            distance_away += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        ant_spot = self.place
        bees_copy = ant_spot.bees[:]
        Ant.reduce_armor(self, amount)
        if ant_spot.ant is None:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting <= 0 and rANTdom_else_none(self.place.bees):
    ...
    def action(self, gamestate):
        ...
        if self.true_queen:
            ...
            while True:
                if s is None:
                    return
                if s.ant and not s.ant.doubled:
                    s.ant.damage *= 2
                    s.ant.doubled = True
                s = s.exit
        else:
